# iBridge Offline Setup Script v3.0
# Complete offline installation with USB drive auto-detection
# Creates accounts, installs applications, and copies specific shortcuts
...
# Installation Control - Set to $false to skip problematic applications
$InstallAnyDesk = $true  # Set to $false if AnyDesk causes hanging issues
...
function Copy-ApplicationFiles {
    ...
    $totalSources = $ApplicationSources.Count + 1  # +1 for Desktop-Mtn folder
    $currentSource = 0
    ...
    foreach ($source in $ApplicationSources) {
        ...
        Start-Sleep -Milliseconds 500  # Brief pause to show progress
    }
    
    # Separately copy only the Desktop-Mtn folder from IT STUFF (excluding unwanted files)
    $currentSource++
...
function Install-Applications {
    ...
    foreach ($installer in $installers) {
        ...
        # Check if we should skip AnyDesk
...
function Force-ProfileCreation {
    ...
    foreach ($profile in $profiles) {
        ...
        # Force create profile directories
...
function Copy-iBridgeUserShortcuts {
    ...
    foreach ($shortcut in $iBridgeUserShortcuts) {
        ...
        Start-Sleep -Milliseconds 300  # Brief pause to show progress
    }
}
...
# Initialize
Write-Host "iBridge Offline Setup v3.0" -ForegroundColor Green
