# Simple Working Script Orchestrator
...
Write-Host "`n📋 EXECUTION PLAN: $($workingScripts.Count) scripts" -ForegroundColor Cyan

foreach ($script in $workingScripts) {
    $fullPath = Join-Path $scriptPath $script
    $scriptName = Split-Path $script -Leaf
    
    Write-Host "`n[$($executed + $failed + 1)] 🔧 $scriptName" -ForegroundColor Yellow
    ...
    if (Test-Path $fullPath) {
        try {
            ...
            if ($LASTEXITCODE -eq 0 -or $null -eq $LASTEXITCODE) {
                Write-Host "✅ $scriptName completed" -ForegroundColor Green
                $executed++
            } else {
                Write-Host "⚠️ $scriptName completed with warnings" -ForegroundColor Yellow
                $executed++
            }
        }
        catch {
...
Write-Host "`n✅ Successfully executed: $executed" -ForegroundColor Green
...
Write-Host "`nSimple orchestrator completed!" -ForegroundColor Green
