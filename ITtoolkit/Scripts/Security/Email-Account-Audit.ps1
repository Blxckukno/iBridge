...
function Test-EmailClientConnections {
    Write-Host "`n📧 EMAIL CLIENT CONNECTION ANALYSIS" -ForegroundColor Yellow
    ...
    if ($emailConnections) {
        ...
        if ($suspiciousConnections) {
            Write-Host "  ⚠️  Suspicious Email Connections:" -ForegroundColor Red
...
function Test-OutlookConfiguration {
    Write-Host "`n📧 OUTLOOK SECURITY CONFIGURATION" -ForegroundColor Yellow
        ...
        if (Test-Path $outlookPath) {
            ...
            if (Test-Path $securityPath) {
                try {
                    ...
                    Write-Host "    ⚠️  Cannot read security settings" -ForegroundColor Yellow
                }
            }
            ...
            if (Test-Path $rulesPath) {
                try {
                    ...
                    if ($rules) {
                        ...
                        foreach ($rule in $rules) {
                            ...
                            if ($ruleName -match "(forward|redirect|delete|move).*external|auto.*reply") {
                                Write-Host "      ⚠️  Potentially suspicious rule: $ruleName" -ForegroundColor Yellow
                    ...
                    Write-Host "    ℹ️  Cannot access email rules" -ForegroundColor Gray
                }
            }
            break
        }
...
function Test-BrowserStoredPasswords {
    Write-Host "`n🔐 BROWSER STORED EMAIL CREDENTIALS" -ForegroundColor Yellow
...
function Test-EmailCertificates {
    Write-Host "`n📜 EMAIL CERTIFICATE ANALYSIS" -ForegroundColor Yellow
    ...
    try {
        ...
        if ($personalCerts) {
            ...
            Write-Host "  ℹ️  No email certificates installed" -ForegroundColor Gray
            $script:recommendations += "Consider using S/MIME certificates for email encryption"
        }
    }
    catch {
        Write-Host "  ⚠️  Cannot access certificate store" -ForegroundColor Yellow
    }
}

function Test-EmailBackupSecurity {
    Write-Host "`n💾 EMAIL BACKUP SECURITY" -ForegroundColor Yellow
    ...
    if ($foundBackups) {
        ...
        foreach ($backup in $foundBackups) {
            ...
            if ($backup.FullName -match "(Desktop|Downloads|Public)") {
                Write-Host "      ⚠️  Located in potentially insecure location" -ForegroundColor Yellow
        ...
        Write-Host "  ℹ️  No email backup files found in common locations" -ForegroundColor Gray
    }
}

function Test-EmailPhishingProtection {
    Write-Host "`n🎣 ANTI-PHISHING CONFIGURATION" -ForegroundColor Yellow
...
function Get-EmailSecurityRecommendations {
    Write-Host "`n💡 COMPREHENSIVE EMAIL SECURITY RECOMMENDATIONS" -ForegroundColor Green
...
try {
    ...
    # Generate summary
    Write-Host "`n📊 EMAIL SECURITY AUDIT SUMMARY" -ForegroundColor White -BackgroundColor DarkGreen
    
    if ($securityIssues.Count -eq 0) {
        Write-Host "  ✅ No critical email security issues detected" -ForegroundColor Green
        $riskLevel = "LOW"
    } elseif ($securityIssues.Count -le 3) {
        Write-Host "  🟡 Minor email security issues detected" -ForegroundColor Yellow
        $riskLevel = "MEDIUM"
    } else {
        Write-Host "  🔴 Multiple email security issues require attention" -ForegroundColor Red
        $riskLevel = "HIGH"
    }
    
    Write-Host "  📊 Risk Level: $riskLevel" -ForegroundColor $(if ($riskLevel -eq "LOW") {"Green"} elseif ($riskLevel -eq "MEDIUM") {"Yellow"} else {"Red"})
    Write-Host "  ⚠️  Issues Found: $($securityIssues.Count)" -ForegroundColor $(if ($securityIssues.Count -eq 0) {"Green"} else {"Yellow"})
...
$(if ($securityIssues) { $securityIssues | ForEach-Object { "⚠️  $_" } | Out-String })

SECURITY RECOMMENDATIONS ($($allRecommendations.Count)):
$(foreach ($rec in $allRecommendations) { "💡 $rec" })
...
"@
    ...
    Write-Host "`n📄 Detailed audit report saved to: $reportPath" -ForegroundColor Yellow
    
} catch {
    Write-Host "`n❌ Error during email security audit: $($_.Exception.Message)" -ForegroundColor Red
}

Write-Host "`n⚡ Email account security audit completed!" -ForegroundColor Green
