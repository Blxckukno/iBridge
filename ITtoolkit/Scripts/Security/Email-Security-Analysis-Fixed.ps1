...
function Test-OutlookSecurity {
    Write-Host ""
    Write-Host "MICROSOFT OUTLOOK ANALYSIS" -ForegroundColor Yellow
    ...
    foreach ($path in $outlookPath) {
        if (Test-Path $path) {
            ...
            Write-Host "  SUCCESS: Outlook found at: $path" -ForegroundColor Green
            break
        }
    }
    
    if (-not $outlookInstalled) {
        Write-Host "  WARNING: Outlook not detected in standard locations" -ForegroundColor Yellow
        return
    }
    ...
    try {
        ...
        Write-Host "  WARNING: Cannot access Outlook security settings" -ForegroundColor Yellow
    }
    ...
    try {
        ...
        Write-Host "  WARNING: Cannot check Outlook add-ins" -ForegroundColor Yellow
    }
}
...
function Test-EmailProcesses {
    Write-Host ""
    Write-Host "EMAIL CLIENT PROCESSES" -ForegroundColor Yellow
    ...
    if ($emailProcesses) {
        ...
        if ($suspiciousEmailProcesses) {
            ...
            $suspiciousEmailProcesses | ForEach-Object {
                Write-Host "    WARNING: $($_.ProcessName) (PID: $($_.Id))" -ForegroundColor Red
...
function Test-EmailAttachments {
    Write-Host ""
    Write-Host "EMAIL ATTACHMENT ANALYSIS" -ForegroundColor Yellow
    ...
    foreach ($location in $attachmentLocations) {
        if (Test-Path $location) {
            ...
            try {
                ...
                if ($recentFiles) {
                    Write-Host "    WARNING: Recent suspicious files:" -ForegroundColor Yellow
                ...
                Write-Host "    WARNING: Cannot scan location" -ForegroundColor Yellow
            }
        }
    }
}
...
function Test-BrowserEmailSecurity {
    Write-Host ""
    Write-Host "BROWSER EMAIL SECURITY" -ForegroundColor Yellow
...
function Test-WindowsMailApp {
    Write-Host ""
    Write-Host "WINDOWS MAIL APP ANALYSIS" -ForegroundColor Yellow
    ...
    if ($mailApp) {
        Write-Host "  SUCCESS: Windows Mail app installed (Version: $($mailApp.Version))" -ForegroundColor Green
        ...
        if (Test-Path $mailDataPath) {
            ...
            try {
                ...
                if ($suspiciousFiles) {
                    Write-Host "    WARNING: Suspicious files in mail data:" -ForegroundColor Red
                ...
                Write-Host "    WARNING: Cannot scan mail data directory" -ForegroundColor Yellow
            }
        }
    } else {
        Write-Host "  INFO: Windows Mail app not installed" -ForegroundColor Gray
    }
}
...
function Get-EmailSecurityRecommendations {
    Write-Host ""
    Write-Host "EMAIL SECURITY RECOMMENDATIONS" -ForegroundColor Green
...
try {
    ...
    # Summary
    Write-Host ""
    Write-Host "EMAIL SECURITY SUMMARY" -ForegroundColor White -BackgroundColor DarkGreen
    
    if ($emailThreats.Count -eq 0) {
        Write-Host "  SUCCESS: No immediate email security threats detected" -ForegroundColor Green
    } else {
        Write-Host "  WARNING: EMAIL SECURITY THREATS DETECTED ($($emailThreats.Count)):" -ForegroundColor Red
...
"@
    ...
    Write-Host ""
    Write-Host "ERROR during email security analysis: $($_.Exception.Message)" -ForegroundColor Red
}

Write-Host ""
Write-Host "Email security analysis completed!" -ForegroundColor Green
