...
try {
    ...
    if ($suspiciousProcesses) {
        ...
        foreach ($proc in $suspiciousProcesses) {
            try {
                ...
                Write-Host "   SUCCESS: Terminated: $($proc.ProcessName) (PID: $($proc.Id))" -ForegroundColor Green
            }
            catch {
                Write-Host "   ERROR: Failed to terminate: $($proc.ProcessName)" -ForegroundColor Red
            }
        }
    }
    ...
    if ($suspiciousConnections) {
        Write-Host "WARNING: SUSPICIOUS NETWORK CONNECTIONS:" -ForegroundColor Red
    ...
    if ($isAdmin) {
        ...
        foreach ($port in $malwarePorts) {
            try {
                ...
                Write-Host "   SUCCESS: Blocked outbound traffic to port $port" -ForegroundColor Green
            }
            catch {
    ...
    if ($suspiciousFiles) {
        Write-Host "WARNING: SUSPICIOUS FILES DETECTED:" -ForegroundColor Red
        $suspiciousFiles | Select-Object Name, FullName, CreationTime, Length | Format-Table -AutoSize
        
        Write-Host "QUARANTINING SUSPICIOUS FILES..." -ForegroundColor Red
        ...
        foreach ($file in $suspiciousFiles) {
            try {
                ...
                Write-Host "   SUCCESS: Quarantined: $($file.Name)" -ForegroundColor Green
            }
            catch {
                Write-Host "   ERROR: Failed to quarantine: $($file.Name)" -ForegroundColor Red
            }
        }
    ...
    Write-Host "2. Enable Windows Defender (run as Administrator)" -ForegroundColor Red
...
"@
    ...
    Write-Host "Error during emergency isolation: $($_.Exception.Message)" -ForegroundColor Red
}
