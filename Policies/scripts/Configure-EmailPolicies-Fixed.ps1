# Email Policy Implementation Script - Fixed Version
param([switch]$WhatIf = $false)

# Load configuration
$config = Get-Content ".\config\email-config.json" | ConvertFrom-Json

Write-Host "=== iBridge Email Policy Implementation (Fixed) ===" -ForegroundColor Cyan
...
foreach ($mailbox in $config.SharedMailboxes) {
    ...
    Set-SharedMailboxPermissions -MailboxIdentity $mailbox.EmailAddress -AuthorizedUsers $config.AuthorizedUsers -ViewOnlyUsers $config.ViewOnlyUsers -WhatIf:$WhatIf
