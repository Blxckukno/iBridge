# Modern IT Support Impact Report Generator
param(
...
try {
    ...
    if ($messages) {
        ...
        foreach ($msg in $messages) {
            ...
            # Track conversation threads for turn around time
            ...
            # Get thread details
            $thread = Get-MessageTrace -MessageTraceId $msg.MessageTraceId
            if ($thread) {
                ...
                if ($duration -gt 0) {
                    $metrics.TurnAroundTimes[$msg.MessageTraceId] = $duration
                }
            }
        }
...
Average Daily Volume: $([math]::Round($metrics.TotalEmails / $metrics.DailyVolume.Count, 2))
"@
