...
const server = http.createServer((req, res) => {
    ...
    const suspiciousPathPattern = /(\.\.|%2e%2e|<script|javascript:|onerror=|onload=)/i;
    if (suspiciousPathPattern.test(decodedPath)) {
        console.warn(`[SECURITY] Blocked suspicious path from ${req.socket.remoteAddress}: ${decodedPath}`);
        setSecurityHeaders(res, 'text/plain; charset=utf-8');
        res.writeHead(403);
        res.end('Forbidden');
        return;
    ...
    fs.readFile(filePath, (err, data) => {
        ...
        } else if (['.css', '.js', '.png', '.jpg', '.jpeg', '.gif', '.svg', '.woff', '.woff2', '.ttf', '.ico'].includes(ext)) {
...
server.listen(PORT, () => {
    ...
    console.log(`👨‍💼 Staff Portal: http://localhost:${PORT}/TicketingSystem/frontend/professional-dashboard.html`);
    console.log(`🎓 LMS Platform: http://localhost:${PORT}/lms-platform.html`);
    console.log(`📊 Security Dashboard: http://localhost:${PORT}/security-dashboard.html`);
